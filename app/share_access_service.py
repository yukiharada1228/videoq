        ...
        try:
            ...
            for session_id in active_sessions:
                session_key = self._get_session_key(
                    share_token, session_id.decode("utf-8")
                )
            ...
            if active_count >= self.max_concurrent_users:
                return (
                    False,
                    "",
                    f"同時アクセス上限（{self.max_concurrent_users}人）に達しました。しばらく時間をおいてから再度アクセスしてください。",
                )
    ...
    def update_session_activity(self, share_token: str, session_id: str) -> bool:
        ...
        try:
            ...
            if session_data:
                ...
                self.redis_client.setex(
                    session_key, self.session_timeout_seconds, json.dumps(session_info)
                )
                return True
