...
連携解除では、その利用者と対象アプリの同意情報・アクセス／リフレッシュトークンのレコードを一つのトランザクションで削除します。認可コードと更新後のリフレッシュトークンには最初の同意IDを `referenceId` として引き継ぎ、JWTには `videoq_grant` として含めます。MCPはリクエストごとに、その同意ID・利用者・アプリ・スコープをDBで確認します。再連携で新しい同意が作られても、古いトークンや認可コードは使えません。

OAuthで対応する認可方式は、認可コードの交換とリフレッシュです。どちらも発行前に最初の同意と現在の利用者の状態を検証し、`resource` を省略して不透明トークンを発行する場合も検証します。UserInfoのGET・POSTにも同じ検証を適用し、認証済みクライアントからのトークン照会では、失効済み・利用停止中のトークンを `active: false` と返します。`resource` を省略する正常なOAuth/OIDC連携は引き続き利用できます。

この同意IDの検証を初めて導入する際、既存のOAuth連携は再認可が必要です。同意IDに紐づかないトークンは拒否します。DBマイグレーションは不要で、ブラウザセッションとAPIキーの形式は変わりません。
