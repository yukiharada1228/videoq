...
可能なら学習の区切りで再構築してください。`ready` の後に新しい問い・ヒントを確認し、Q&A → Study と切り替えて古い会話表示を消してから、対象の概念について新しい問いへの取り組みを始めます。モード切り替えはセッション全体の進捗リセットではありません。再構築前の経路から、そのまま続けられるとは案内しないでください。
