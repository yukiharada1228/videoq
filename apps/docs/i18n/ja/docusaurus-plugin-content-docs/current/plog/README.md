...
手修正では概念IDとセッション内の進捗が残ります。手修正・再構築後の最初の問いとヒントを確認するには、新しい確認用セッションを開始します。

1. 講座または確認に使う共有URLを開き、**Study（学習）** を選びます。
2. **最初からやり直す** を選び、確認します。このタブ・経路の会話表示と未送信の入力が消え、進捗が空のセッションに切り替わります。保存済みチャット履歴は残り、講座所有者が確認できます。
3. 対象の概念について尋ねます。前提概念が先に出た場合は順に取り組んでから、対象概念の最初の問いと続くヒントを、編集した学習グラフと照合します。

元のタブの進捗を残したい場合は、そのタブを開いたまま、ブラウザーの**新しいタブ**（`Ctrl+T`／`Cmd+T`）に同じURLを貼り付け、そちらで上の手順を行います。タブの複製・復元や、開いた元のページとの関連（opener）を残すリンクでは、セッションIDがコピーされる場合があります。詳しくは[学習の再開とやり直し](study-sessions.md)を参照してください。

再読み込みやQ&A → Studyの切り替えだけでは、新しいセッションになりません。元のセッションを使うと、以前の会話が画面に見えなくても、最初の問いを飛ばしたり、確認用の発言を以前の問いへの返答として採点したりする場合があります。
