...
Q&Aは、各リクエストで対象を指定し、前の回答を引き継がずに根拠を検索する単発質問の機能として維持します。認証済みのチャット、共有リンク、講座を指定しないQ&Aに共通です。ブラウザーは `user` のメッセージを1件だけ送信します。ストリーミングAPI（`/api/chat/messages/stream`）と `chat.send` は互換性とStudyのために `messages` 配列を受け付けますが、Q&Aでは最後の `user` 発言だけを選び、それより前の発言を文脈として使いません。`mode` を省略すると `qa` です。
...
Studyだけは、直前のAIの問いを参照するため、直近の会話を送信します。新しい返答を含む空でない最大12件で、最初の挨拶は除きます。どちらのモードでも、保存済みのチャットログをモデル入力として読み直すわけではありません。
