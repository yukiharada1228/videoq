...
Studyに保存された問い・ヒントは、字幕保存や検索再索引では更新しません。影響する学習データを確認して編集するか、残したい手編集を控えてからPLOGを明示的に再構築します。再構築は手編集を置き換え、進行中の学習を新しい概念IDへ引き継ぎません。操作を選ぶ前に[更新範囲の表と字幕修正の手順](../architecture/transcription-and-search.md#update-scope)を確認してください。
