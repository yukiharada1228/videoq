...
シーン検索・索引作成・PLOG概念の埋め込み・Studyの採点を行う経路では、DBの宣言型が `vector(1536)` か確認します。空テーブルでも検出し、列欠落・別型・次元未固定の `vector` は拒否します。

生成ベクトルは、1536個の有限な数値からなり、pgvectorのfloat32に収まり、非ゼロである必要があります。文字列や真偽値を数値に変換して受け入れません。バッチ件数とOpenAIの応答indexも入力に対応するか検証します。Studyは保存済みPLOGの非空ベクトルも検証します。未生成の空ベクトルは既存の未準備扱いを維持し、不正データを類似度0や学習者の誤答に置き換えません。
...
今回、既存データのモデル移行ツールは提供していません。1536次元のままモデルを変える場合も、シーンとPLOG概念の両方を再生成する必要があります。現行の全体再索引はシーンのみが対象です。PLOG全体再構築は概念ID・手編集した教材・学習履歴を置き換える可能性があり、どちらもモデル移行の代わりにはなりません。
...
3. 保守時間を確保し、Q&A・Study・workerの書き込み・APIのPLOG編集・実行中処理・キュー内の旧ジョブを止める。
4. 字幕・概念・手編集教材・ベクトル・スキーマ・アプリ版・設定を復元できるバックアップを確保する。次元の異なる旧ベクトルを `ALTER TYPE` だけで保持できるとは扱わない。
5. 概念ID・ラベル・関係・問い・ヒント・学習履歴を維持し、シーンと概念のベクトルを同じ構成で再生成する。
6. 件数・欠落・次元・Q&A検索・Studyの概念選択を確認し、API・workerの設定を揃えて再開する。途中失敗時は保守を維持し、旧データ・スキーマ・アプリ・設定を一式で復元する。

教材保持と復旧の仕組みが揃うまで、既存DBのモデル・次元変更は標準の運用手順として提供しません。破棄可能な開発DBの作り直しとは区別してください。
