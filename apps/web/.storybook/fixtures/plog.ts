...
export const edgeTypes = ['prerequisite_of', 'builds_on', 'analogy_for', 'example_of', 'contrasts_with', 'presentation_order'] as const;
export const edges: PlogEdge[] = edgeTypes.map((edge_type, index) => ({
  ...
  provenance: index === 5 ? 'generated' : index === 1 ? 'edited' : 'unknown',
  ...(index === 5 ? { source_id: 1, source_label: concepts[0].label, target_id: 3, target_label: concepts[2].label, quote: '' } : {}),
}));
...
export const englishGraph: PlogGraph = {
  ...
  edges: edges.map(item => ({ ...item, source_label: ['Rotation matrix', 'Length preservation', 'Approximation limits'][item.source_id - 1], target_label: ['Rotation matrix', 'Length preservation', 'Approximation limits'][item.target_id - 1], quote: item.quote ? 'Compare the examples in the lecture.' : '' })),
};
