...
スキーマ正本は Drizzle（`src/db/schema` + `drizzle/0000_init.sql`）、repositories は query builder（`withDb`）、非同期ジョブは [`apps/worker/`](../worker/)（Django/Celery なし）。旧 Django ツリーは [`archive/django-backend/`](../../archive/django-backend/)。マイグレーション: `npm run db:migrate`（stamp + migrate）。
...
| **`POST /api/videos/`** | API キー + Cookie/Bearer JWT | **`USE_S3_STORAGE=false`**: multipart → `VIDEO_BUCKET.put(media/...)` → pending → transcription。**`USE_S3_STORAGE=true`（ローカル MinIO / 本番 R2 既定）**: 廃線。400 で `/uploads/` を案内 |
| **`POST /api/videos/uploads/`** | API キー + Cookie/Bearer JWT | **`USE_S3_STORAGE=true` のみ**。署名 URL（MinIO / R2）。false は 400。serializer + allowlist + `FILE_TOO_LARGE` / `STORAGE_LIMIT_EXCEEDED`。保留動画 + presigned PUT。file key は `videos/{uid}/video_{ms}_{reservedBytes}{ext}`（**FR-Q3**）。201 `{video, upload_url}` |
...
> **`file` URL（`src/integrations/media.ts`）**: `USE_S3_STORAGE=true`（wrangler 既定）→ S3 互換 presigned GET（aws4fetch SigV4, path-style, `media/` 前置, expire 3600）。**ローカルは MinIO**（`docker compose up -d minio minio-init` + `.dev.vars` の `R2_*`）。本番は Cloudflare R2（同じ変数、`R2_S3_REGION=auto`）。`USE_S3_STORAGE=false` → `/api/media/{file_key}`（`VIDEO_BUCKET`）。フロントは `VITE_USE_S3_STORAGE=true` のとき署名 PUT 3 ステップ。
...
### ローカル開発（Postgres + MinIO + ElasticMQ）

```bash
# リポジトリルート — Postgres / MinIO / ElasticMQ / worker poller
docker compose up -d postgres minio minio-init elasticmq worker
# MinIO :9000  ElasticMQ :9324  worker logs: docker compose logs -f worker

cd apps/api
cp .dev.vars.example .dev.vars   # JWT_SECRET / R2_*（MinIO）/ SQS_*（ElasticMQ）
npm run dev                      # http://localhost:8787
```

フロントは `frontend/.env.local` に `VITE_API_URL=/api` と `VITE_USE_S3_STORAGE=true`。

- `/ready` は Hyperdrive `localConnectionString`（例: `127.0.0.1:55432`）の Postgres に接続する。
...
| vars | `ENVIRONMENT` / `USE_S3_STORAGE` / `CORS_ALLOW_ORIGIN` | 非機密（ローカル既定 `USE_S3_STORAGE=true` + MinIO。エンドポイント等は `.dev.vars`） |
