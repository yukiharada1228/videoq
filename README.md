...
- ユビキタス言語ガイド: [docs/architecture/ubiquitous-language.md](docs/architecture/ubiquitous-language.md)

### 仕様会話/PRルール（必須）

- 最初に文脈を明示: `Auth` / `Video` / `Chat`
- 文脈が跨る同名語は接頭辞で区別: `Video group` / `Chat group context`
- PR には「追加/変更したドメイン語彙」を記載
- 曖昧語 (`group` / `token` / `user`) を単体で使わない
- テンプレート:
  - PR(backend): `.github/PULL_REQUEST_TEMPLATE/backend.md`
  - Issue(仕様会話): `.github/ISSUE_TEMPLATE/spec_discussion.yml`
