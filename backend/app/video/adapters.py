...
class UploadVideoAdapter:
    def __init__(self, *, actor_loader, video_creator):
...
class AddVideoToGroupAdapter:
    ...
    def __call__(self, command: AddVideoToGroupCommand):
        user = self._actor_loader(command.actor_id)
        group = self._owned_resource_loader(user, self._group_model, command.group_id)
        if not group:
            raise LookupError("Group not found")
        video = self._owned_resource_loader(user, self._video_model, command.video_id)
        if not video:
...
class AddVideosToGroupAdapter:
    ...
    def __call__(self, command: AddVideosToGroupCommand):
        ...
        videos = self._owned_resources_loader(user, self._video_model, command.video_ids)
...
class ReorderVideosInGroupAdapter:
    def __init__(self, *, actor_loader, group_model, owned_resource_loader, group_reorderer):
        self._actor_loader = actor_loader
        self._group_model = group_model
        self._owned_resource_loader = owned_resource_loader
...
class DeleteShareLinkAdapter:
    def __init__(self, *, actor_loader, group_model, owned_resource_loader, share_token_updater):
        self._actor_loader = actor_loader
        self._group_model = group_model
        self._owned_resource_loader = owned_resource_loader
        self._share_token_updater = share_token_updater
...
class AddTagsToVideoAdapter:
    ...
    def __call__(self, command: AddTagsToVideoCommand):
        ...
        tags = self._owned_resources_loader(user, self._tag_model, command.tag_ids)
