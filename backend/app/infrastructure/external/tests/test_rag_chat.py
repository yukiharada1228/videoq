...
def _force_classic_rag(mock_agent_cls) -> None:
    """Make QaToolAgent fail before emitting so gateway uses classic RAG."""
    mock_agent = MagicMock()
    mock_agent.run.side_effect = RuntimeError("force classic rag")
    mock_agent.stream.side_effect = RuntimeError("force classic rag")
    mock_agent_cls.return_value = mock_agent


class RagChatGatewayStreamingTests(TestCase):
    """Tests for RagChatGateway.stream_reply() classic-RAG fallback path."""
    ...
    def test_stream_reply_yields_content_chunks(
        self, mock_agent_cls, mock_service_cls, mock_get_llm
    ):
        _force_classic_rag(mock_agent_cls)
    ...
    @patch("app.infrastructure.external.rag_gateway.QaToolAgent")
    @override_settings(LLM_PROVIDER="openai")
    def test_stream_reply_splits_batched_service_chunks_into_single_units(
        self, mock_agent_cls, mock_service_cls, mock_get_llm
    ):
        _force_classic_rag(mock_agent_cls)
        mock_get_llm.return_value = MagicMock()
    ...
    @patch("app.infrastructure.external.rag_gateway.QaToolAgent")
    @override_settings(LLM_PROVIDER="openai")
    def test_stream_reply_yields_final_chunk(
        self, mock_agent_cls, mock_service_cls, mock_get_llm
    ):
        _force_classic_rag(mock_agent_cls)
        mock_get_llm.return_value = MagicMock()
    ...
    @patch("app.infrastructure.external.rag_gateway.QaToolAgent")
    @override_settings(LLM_PROVIDER="openai")
    def test_stream_reply_wraps_runtime_errors_as_llm_provider_error(
        self, mock_agent_cls, mock_service_cls, mock_get_llm
    ):
