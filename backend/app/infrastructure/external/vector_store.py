"""
PGVector infrastructure: manages PGVectorStore operations.
...
class PGVectorManager:
    ...
    @classmethod
    def create_vectorstore(cls, embeddings) -> PGVectorStore:
    ...
    @classmethod
    def _get_management_store(cls) -> PGVectorStore:
...
def delete_video_vectors(video_id: int) -> None:
    """Delete vector data related to the specified video ID."""
    try:
...
def update_video_title_in_vectors(video_id: int, new_title: str) -> int:
    """
    Update video_title in PGVectorStore metadata using raw SQL.
