...
beforeEach(() => {
  ...
  queryMock.mockReset().mockImplementation((sql: string) => {
    ...
    if (sql.includes("SELECT storage_limit_gb")) {
      return { rows: [{ storage_limit_gb: null, is_over_quota: false }], rowCount: 1 };
    }
    if (sql.includes("UPDATE app_user SET used_storage_bytes")) {
      return { rows: [], rowCount: 1 };
    }
    if (sql.includes("INSERT INTO app_video")) {
      return { rows: [{ id: 42 }], rowCount: 1 };
    }
    if (sql.includes("SELECT v.id") && sql.includes("FROM app_video v")) {
      return {
        rows: [
          {
            id: 42,
