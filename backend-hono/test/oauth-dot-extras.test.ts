...
describe("RFC 7662 introspect", () => {
  ...
  it("returns active token for authenticated client", async () => {
    ...
    rowsFor = (sql) => {
      ...
      if (sql.includes("FROM oauth2_provider_accesstoken")) {
        expect(sql).toContain("token_checksum");
        return [
          {
            scope: "read",
            exp: 1_700_000_000,
  ...
  it("allows Bearer with introspection scope", async () => {
    rowsFor = (sql, args) => {
      if (
        sql.includes("SELECT scope FROM oauth2_provider_accesstoken") &&
        sql.includes("token_checksum")
      ) {
        return [{ scope: "read introspection" }];
      }
      if (sql.includes("LEFT JOIN oauth2_provider_application")) {
        return [
          {
            scope: "read",
            exp: 1_700_000_001,
  ...
  it("rejects Bearer without introspection scope", async () => {
    rowsFor = (sql) => {
      if (sql.includes("SELECT scope FROM oauth2_provider_accesstoken")) {
        return [{ scope: "read" }];
      }
      return [];
    };
...
describe("RFC 8628 device authorization + token poll", () => {
  ...
  it("issues device_code and user_code", async () => {
    ...
    rowsFor = (sql) => {
      ...
      if (sql.includes("INSERT INTO oauth2_provider_devicegrant")) {
        return [grant];
      }
      return [];
    };
  ...
  it("token poll returns authorization_pending then tokens", async () => {
    ...
    rowsFor = (sql) => {
      ...
      if (sql.includes("FROM oauth2_provider_devicegrant")) {
        return [
          deviceGrant({
            status: phase,
...
describe("applications / authorized_tokens HTML", () => {
  ...
  it("lists applications for cookie user", async () => {
    rowsFor = (sql) => {
      if (sql.includes("FROM oauth2_provider_application") && sql.includes("user_id")) {
