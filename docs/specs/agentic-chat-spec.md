...
2. **会話履歴の活用**: 「もっと長く」「具体的に」等のフォローアップ質問を機能させる(現状は最新発言 1 件しか使っていない)。
3. **既存契約の温存**: `RagGateway` ポート・`CitationDTO`・序数引用・フロントのナビゲーション契約・RAGAS 評価インターフェースを**変えずに**差し替え可能にする。
4. **段階的かつ安全な移行**: フィーチャーフラグで新旧を共存させ、グループ単位 A/B、即時ロールバックを可能にする。
...
- **会話履歴の破棄**: `rag_service.py:73,113`(`_extract_latest_user_query`)と `rag_service.py:139-145` が最新 user 発言 1 件のみを使う。プロンプトテンプレート `rag_service.py:55-60` も `("system", ...)`+`("human", "{query_text}")` の 2 メッセージ固定。履歴は `use_cases/chat/send_message.py:103` で全件 `ChatMessageDTO` 化されゲートウェイ入口まで届いているが、`rag_service` が捨てている。**修正は infrastructure 側で完結する。**
...
```
[Client]
  │  POST /api/chat/messages(/stream/)  messages[]=会話履歴全件 + group_id
  ▼
...
[domain/chat/gateways.py]  RagGateway (ABC)  ← シグネチャ不変
  ...
  └── 新規: AgenticChatGateway (エージェント型)  ← USE_AGENT_CHAT 設定時 / 対象グループ
   ...
   │ 1. 会話履歴全件 → LangChain メッセージ列 + system(ツール使用ルール+group_context) │
...
| ツール | 理由 |
|---|---|
| `get_chat_history` | 会話履歴は LLM の message 配列で既に保持 |
...
履歴は presentation → use_case → gateway まで全件流れている(`send_message.py:103` で全件 `ChatMessageDTO` 化)。捨てているのは `rag_service.py:73,113`(latest-only 抽出)と `:55-60`(2 メッセージ固定テンプレート)。**修正は infrastructure 側で完結**。

### 6.2 新ゲートウェイでの履歴変換

`messages: Sequence[ChatMessageDTO]` 全件を LangChain メッセージ列へ変換し、ツールループの会話バッファ初期状態とする。

```python
def _to_langchain_messages(messages, system_prompt, max_history_turns, max_history_tokens) -> list[BaseMessage]:
    # system は先頭に1つだけ(build_system_prompt で生成)。
    # user/assistant を古い順に保ち、新しい側から max_history_turns / max_history_tokens の
    # 小さい方で打ち切る。user/assistant 以外の role は無視。
```
...
- **履歴は新しい側を優先**(直近文脈ほど重要)。先頭(古い)から落とす。
- ツール往復(`AIMessage(tool_calls=...)` / `ToolMessage`)はその**ターン内**バッファのみに追加し、永続履歴(`ChatLog`)には保存しない(トレースは `tool_trace` で別保存, §7.3)。

### 6.3 設定値(env)

| env | 既定 | 用途 |
|---|---|---|
| `CHAT_MAX_HISTORY_TURNS` | 12 | 履歴として残す user+assistant 件数の上限 |
| `CHAT_MAX_HISTORY_TOKENS` | 4000 | 履歴トークン予算(超過で古い側から落とす) |

`RagGateway` のシグネチャ(`gateways.py:53-90`)は `messages` を既に受けるため**インターフェース変更不要**。
...
| 入力トークン | system+query+20 チャンク | system+履歴+ツール結果(全文/要約で急増) |

### 7.2 全文 transcript のサイズゲート(二段構え)

`get_video` ツールラッパ内でサイズに応じて分岐:

| transcript トークン概算 | 返却内容 |
|---|---|
...
- トークン概算は `len(text) / 2.5` 近似(日本語含む。`tiktoken` は重いので導入しない)。
- ツール選択は LLM 任せ、サイズゲートはサーバ側で強制(全文連打のコスト爆発を防止)。

#### 7.2.1 長尺の map-reduce 要約

`transcript_summarizer.py`:
- `chunk_transcript_srt(srt_text, max_chunk_tokens=1500) -> list[TranscriptChunk]`: SRT 字幕ブロック境界で結合・分割。各チャンクに start/end(SRT)と start_sec/end_sec を保持。
...
| 全文 1 回の切り詰め | `TRANSCRIPT_CHAR_BUDGET` | 24000 字 | 末尾に `…(truncated)` を付与 |
...
| LLM プロンプトプレフィックス | provider 依存(Anthropic prompt caching 等) | provider 管理 | system→古い履歴→新しい履歴→ツール往復 の順序固定でキャッシュヒット率を上げる |
...
§7.3 の三重上限(反復回数・総呼び出し数・get_video 専用)+ トークン/文字バジェット + 同一呼び出し重複検知 + wall-clock タイムアウト。到達時はツール無しで最終回答を強制(`_force_answer`)。
...
会話文脈の回帰: 「もっと長く」「具体的に」のフォローアップで agent が履歴を文脈に使うことを `test_stream_message.py` 流の複数ターン use_case テストで検証(`rag_service.py:139-145` の latest-only 抽出が agent 経路で起きないこと)。
