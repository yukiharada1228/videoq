...
Choose **invitations** for access tied to named accounts, or a **share link** when anyone holding the URL may use the course without signing in. Both let learners watch course videos and use Q&A and Study. **Their questions use the course owner's AI answer allowance, and the owner can read and export the saved conversations.**
...
| Use Study | Yes, with a usable PLOG | Yes, with a usable PLOG | Yes, with a usable PLOG |
| Edit/delete the course, change its videos or their order | Yes | No | No |
| Edit the source video, transcript, or PLOG | Video owner only | No permission granted by membership | No permission granted by the link |
...
Playback access applies to videos in the course. It does not grant access to the owner's whole video library or the source video's editing screen. Study also requires a ready, usable [learning graph](../plog/README.md); permission to chat does not guarantee that graph is available.

Participants and link users can see answers in their current chat panel, but cannot load saved course history or export even just their own past questions through the history API. The owner-only history includes Q&A and Study exchanges. Visible messages, saved chat logs, and Study's temporary progress are different data.
...
1. The API checks B's access and reserves one response from **A's** AI answer allowance before answering. B's own allowance is not charged. This also applies to Study responses, including its saved opening question.
...
| Delete course chat history | Saved questions, answers, feedback, and their evaluations are deleted | Course access and membership; this is not a Study-progress reset |
...
3. Verify with a separate learner account or a signed-out browser, according to the chosen route. Check playback and chat, and the absence of course-editing and saved-history controls. A new Q&A/Study response uses the owner's allowance.
